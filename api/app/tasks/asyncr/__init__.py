...
@celery.task(bind=True)
def async_handle_transaction_input_data_upload(self, file_path_in: str, file_type: str, df_encoding: str, delimiter: str, basepath: str, user_id: int, seller_firm_id: int, seller_firm_notification_data: SellerFirmNotificationInterface):
    from app.namespaces.transaction_input.service import TransactionInputService
    response_object = TransactionInputService.handle_transaction_input_data_upload(file_path_in, file_type, df_encoding, delimiter, basepath, user_id, seller_firm_id, seller_firm_notification_data)
