...
@ns.route("/<int:distance_sale_id>")
@ns.param("distance_sale_id", "DistanceSale database ID")
class DistanceSaleIdResource(Resource):
    def get(self, distance_sale_id: int) -> DistanceSale:
        """Get Single DistanceSale"""
        return DistanceSaleService.get_by_id(distance_sale_id)

    def delete(self, distance_sale_id: int) -> Response:
        """Delete Single DistanceSale"""
        from flask import jsonify

        id = DistanceSaleService.delete_by_id(distance_sale_id)
        return jsonify(dict(status="Success", id=id))

    @ns.expect(distance_sale_dto, validate=True)
    @ns.marshal_with(distance_sale_dto)
    def put(self, distance_sale_id: int) -> DistanceSale:
        """Update Single DistanceSale"""
        data_changes: DistanceSaleInterface = request.parsed_obj
        return DistanceSaleService.update(distance_sale_id, data_changes)
