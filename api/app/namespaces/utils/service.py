...
class HelperService:
    ...
    def stringify(string_raw: str) -> str:
        ...
        string = string_raw.lower().translate(
            {ord(c): " " for c in "!@#$%^&*()[]{};:,./<>?\|`~-=_+"}).replace(" ", "-").strip("-")
...
class HistoryService:
    ...
    def handle_update(obj_id: int, HistoryClass, ObjHistoryService, data_changes: Dict):
        ...
        history_obj = ObjHistoryService.get_by_relationship_date(
            obj_id, valid_from)
...
class NotificationService:
    ...
    def get_seller_firm_shared(seller_firm_id: int, user_id: int, time: datetime, subject: str) -> SellerFirmNotification:
        ...
        seller_firm_notification = SellerFirmNotification.query.filter(
            ...
            SellerFirmNotification.created_on.between(
                time - timedelta(minutes=timespan_in_min), time),
    ...
    def handle_seller_firm_notification_data_upload(seller_firm_id: int, user_id: int, tag: str, seller_firm_notification_data: SellerFirmNotificationInterface) -> None:
        ...
        seller_firm_notification = NotificationService.get_seller_firm_shared(
            seller_firm_id, user_id, datetime.utcnow(), subject='Data Upload')
        if not isinstance(seller_firm_notification, SellerFirmNotification):
            seller_firm_notification = NotificationService.create_seller_firm_notification(
...
class InputService:
    ...
    def determine_file_type(df: pd.DataFrame) -> str:
        # new platform input files are being specified here for inambiguous identification
        ...
        if ('channel_code' in column_name_list and 'channel_code' in column_name_list):
            ...
            return 'transactions_amazon_2020'
              ...
              and 'ACTIVITY_PERIOD' in column_name_list
              and 'ASIN' in column_name_list
              and 'PROGRAM_TYPE' in column_name_list
              and 'QUANTITY' in column_name_list
              and 'DEPARTURE_CITY' in column_name_list
              and 'ASIN' in column_name_list
    ...
    def infer_delimiter(file_path_in: str) -> str:
        reader = pd.read_csv(file_path_in, sep=None,
    ...
    def store_file(file: BinaryIO, allowed_extensions: List[str], basepath: str, business_id: int, file_type: str) -> str:
        if InputService.allowed_file(filename=file.filename, allowed_extensions=allowed_extensions):
            ...
            basepath_in = os.path.join(
                basepath, str(business_id), file_type, 'in')
            ...
            try:
                ...
                raise UnprocessableEntity(
                    'Can not store file. Please contact one of the admins.')
            ...
            if InputService.allowed_filesize(file_path=file_path_in):
                ...
                raise RequestEntityTooLarge(
                    'The uploaded file "{}" exceeds the file limit.'.format(stored_filename))

        else:
            raise UnsupportedMediaType('The file type "{}" is not allowed. Please recheck if the file extension matches one of the following: {}'.format(
    ...
    def move_data_to_file_type(file_path_tbd: str, file_type: str, business_id: int):
        ...
        basepath_file_type = os.path.join(
            basepath, str(business_id), file_type, 'in')
    ...
    def move_file_to_out(file_path_in: str, basepath: str, file_type: str, business_id: int):
        ...
        basepath_out = os.path.join(
            basepath, str(business_id), file_type, 'out')
        ...
        if os.path.isfile(file_path):
            ...
            try:
                if file_name.lower().endswith('.csv') or file_name.lower().endswith('.txt'):
                    df_raw = pd.read_csv(
                        file_path, encoding=df_encoding, delimiter=delimiter)
            ...
            raise InternalServerError(
                'Not a path: {}'.format(file_path))  # !!! (not a file)
    ...
    def clean_df(df_raw: pd.DataFrame) -> pd.DataFrame:
        ...
        # removes , from both ends of column
