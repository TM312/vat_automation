          ...
          <b-table-lite borderless fixed small :items="itemsCategory" :fields="fieldsCategory" class="mb-4">
            <template v-slot:cell(value)="data">
              <span v-if="Array.isArray(data.value)===true">
                ...
                <b-popover target="popover-vatin" triggers="hover" placement="top" class="ml-2" :title="`${vatNumberByCountryCode.country_code}-${vatNumberByCountryCode.number}`">
                  <div>Valid: <b-icon :variant="vatNumberByCountryCode.valid ? 'success':'danger'" :icon="vatNumberByCountryCode.valid ? 'check-circle':'x-circle'" /></div>
                  <div>Request Date: {{ vatNumberByCountryCode.request_date }} </div>
                </b-popover>
                <b-badge id="popover-vatin" variant="secondary" class="ml-2">{{ data.value[1] }}-{{ data.value[2] }}</b-badge>
              </span>
        ...
        <div class="mt-5">
            ...
            <b-row>
              ...
              <b-col cols="7" xl="8" style="white-space: pre-wrap word-wrap:break-word" class="mr-auto">
                <b-button-group size="sm" class="ml-2">
                  <b-button :variant="transaction.supplier_relationship === 'B2B' ? 'success' : 'outline-secondary'">
                    B2B
                  </b-button>
                  <b-button :variant="transaction.supplier_relationship === 'B2C' ? 'success' : 'outline-secondary'">
                    B2C
                  </b-button>
                </b-button-group>
              </b-col>
          ...
          <b-card v-else title="Customer">
            <b-row>
              ...
              <b-col cols="7" xl="8" style="white-space: pre-wrap word-wrap:break-word" class="mr-auto">
                <b-button-group size="sm" class="ml-2">
                  <b-button :variant="transaction.customer_relationship === 'B2B' ? 'success' : 'outline-secondary'">
                    B2B
                  </b-button>
                  <b-button :variant="transaction.customer_relationship === 'B2C' ? 'success' : 'outline-secondary'">
                    B2C
                  </b-button>
                </b-button-group>
              </b-col>
    ...
    export default {
        ...
        computed: {
            ...
            vatNumberByCountryCode() {
                return this.$store.getters['seller_firm/vatNumberByCountryCode'](this.transaction.tax_jurisdiction_code)
            },

            taxJurisdiction() {
                return this.$store.getters['country/countryNameByCode'](this.transaction.tax_jurisdiction_code)
            },
            ...
            itemsCategory() {
                return [
                    ...
                    { category: 'Tax Jurisdiction', value: [this.taxJurisdiction, this.vatNumberByCountryCode.country_code, this.vatNumberByCountryCode.number]}
                ]
            },
