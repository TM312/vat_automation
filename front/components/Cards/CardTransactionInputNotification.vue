<template>
    <b-card :border-variant="notification.status" :title="capitalize(notification.status)" :sub-title="notification.subject" >
        ...
        <b-table :items="items" :fields="fields" borderless>
             <template v-slot:cell(key)="data"><b>{{ data.value }}</b></template>
        </b-table>

        <p v-if="notification.message" class="my-2">{{ notification.message }}</p>

        <b-card-text class="small text-muted">
            <b-row>
                <b-col cols="auto" class="mr-auto"><span>{{ notification.original_filename }}</span></b-col>
                <b-col cols="auto">
